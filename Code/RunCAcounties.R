...
RunOneCounty <- function(county1, county.dt, county.pop, quick.test) {
  restart.set <- c("Tehama", "Mono", "Yolo", "Yuba", "Mendocino", "Nevada", "El Dorado", 
                   ...
                   "Madera", "Humboldt", "Siskiyou", "Butte", "San Benito") #infections went to near zero - restart sim
  if (county1 %in% restart.set) {
    if (county1 %in% c("San Benito")) {
...
if (quick.test) county.set <- c("San Benito", "Solano", "Stanislaus", "Tulare", "Yolo")
print(county.set)
